fetchSocialData = (periode)->
  ...
  MongoClient.connect config.mongoURL, (err, db)->
    ...
    collection.find().sort({ 'CreatedAt': -1 }).limit(10).toArray (err, results)->
    ...
    collection.find().sort({ 'PublishedAt': -1 }).limit(10).toArray (err, results)->
    ...
    collection.find().sort({ 'ID': -1 }).limit(10).toArray (err, results)->
